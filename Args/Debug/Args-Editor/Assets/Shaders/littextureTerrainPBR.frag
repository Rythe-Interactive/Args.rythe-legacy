...
{
	vec3 blendingWeights = pow(abs(normal.xyz), vec3(4));
...
vec4 MixLayers(sampler2DArray map, vec2 uvOffset, vec3 position, vec3 normal, vec4 layers)
{
	vec4 layer0 = vec4(0);
	if(layers.r > 0)
		layer0 = TriplanarMap(map, uvOffset, position, normal, 0);

	vec4 layer1 = vec4(0);
	if(layers.g > 0)
		layer1 = TriplanarMap(map, uvOffset, position, normal, 1);

	vec4 layer2 = vec4(0);
	if(layers.b > 0)
		layer2 = TriplanarMap(map, uvOffset, position, normal, 2);

	vec4 layer3 = vec4(0);
	if(layers.a > 0)
		layer3 = TriplanarMap(map, uvOffset, position, normal, 3);
...
void main( void )
{
	...
	fragmentNormal = surfaceNormal;

	viewDirection = normalize(cameraPosition - surfacePosition);

	parallaxOffset = ParallaxMap(MixLayers(heightMap, vec2(0.0), surfacePosition, surfaceNormal, weights).r, heightScale * 0.015, tbnMatrix * viewDirection);

 	albedo = pow(MixLayers(albedoMap, parallaxOffset, surfacePosition, surfaceNormal, weights).rgb, vec3(2.2));
 	fragmentNormal = CalculateNormal(MixLayers(normalMap, parallaxOffset, surfacePosition, surfaceNormal, weights).rgb, tbnMatrix);
